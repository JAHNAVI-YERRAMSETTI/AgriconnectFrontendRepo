// src/farmer/UpdateFarmerProfile.jsx
...
export default function UpdateFarmerProfile() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.id) {
      setError("Missing Farmer ID. Cannot update profile.");
      return;
    }

    try {
      const response = await axios.put(`${config.url}/farmer/updateprofile`, formData);
      if (response.status === 200) {
        setMessage(response.data);
        setError('');
        sessionStorage.setItem('farmer', JSON.stringify(formData));
      }
    } catch (error) {
      setMessage('');
  ...
  return (
    <div>
      <h3 style={{ textAlign: "center", textDecoration: "underline" }}>Update Farmer Profile</h3>
